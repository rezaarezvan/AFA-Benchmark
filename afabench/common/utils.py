...
#     method_class = get_class(metadata["method_class_name"])
...
#     dataset_class = get_class(metadata["class_name"])
...
def load_dataset_splits(
    ...
    for f in ["train.pt", "val.pt", "test.pt", "metadata.json"]:
        if not (dataset_artifact_path / f).exists():
            ...
            raise FileNotFoundError(msg)
    metadata = load_artifact_metadata(dataset_artifact_path)
    cls = get_class(metadata["class_name"])
...
#     classifier_class = get_class(metadata["class_name"])
