## Dataframe produced by evaluation function and script

This is what the `eval_afa_method` function in `eval.py` produces, which is also what `eval_afa_method.py` returns:
...
This is what `transform_eval_data.py` produces for plotting (and also what `plot_eval.R` expects).
