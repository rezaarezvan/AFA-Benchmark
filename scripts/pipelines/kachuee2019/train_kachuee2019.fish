...
argparse "dataset=+" "budgets=+" "split=+" "help" "launcher=?" "device=?" "dataset-alias=?" "output-alias=?" "wandb-entity=?" "wandb-project=?" -- $argv
or exit 1

# Print help if specified
if set -ql _flag_help
    echo "Usage: train_kachuee2019.fish --dataset=<str> --budgets=<str> --split=<int> [--help] [--launcher={custom_slurm,basic}] [--device={cuda,cpu}] [--dataset-alias=<str>] [--output-alias=<str>] [--wandb-entity=<str>] [--wandb-project=<str>]" >&2
    exit 1
end
...
# --- METHOD TRAINING ---
echo "Starting kachuee2019 method training jobs..."
sleep 1
...
for dataset in $datasets
    ...
    set -a jobs "uv run scripts/train_methods/train_kachuee2019.py -m output_artifact_aliases=[\"$output_alias\"] dataset@_global_=$dataset dataset_artifact_name="(string join , $dataset_artifact_names)" hard_budget="(string join , $dataset_budgets)" $extra_opts"
    set i (math "$i+1")
end
