...
def test_is_permutation_invariant():
    ...
    input_set_11 = torch.tensor([[1, 2], [3, 4], [5, 6], [7, 8], [0, 0]], dtype=torch.float32)
    length_11 = torch.tensor(4, dtype=torch.int64)
    input_set_12 = torch.tensor([[8, 9], [10, 11], [11, 12], [0, 0], [0, 0]], dtype=torch.float32)
    ...
    input_set_21 = torch.tensor([[3, 4], [1, 2], [7, 8], [5, 6], [0, 0]], dtype=torch.float32)
    length_21 = torch.tensor(4, dtype=torch.int64)
    input_set_22 = torch.tensor([[11, 12], [10, 11], [8, 9], [0, 0], [0, 0]], dtype=torch.float32)
    ...
    assert torch.allclose(output_1, output_2), "The outputs are not equal for the same input data with different permutations."
