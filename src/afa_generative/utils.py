...
def base_generate_mask_incomplete(batch_data, mask_prop=0.25, rs=40):
    ...
    for row_idx in range(batch_data.shape[0]):
        pos = np.where(np.abs(batch_data[row_idx, :]) > 0)[
            0
        ]  # Positions of all observed data
        pos_train, pos_test, pos_train, pos_test = train_test_split(
            pos, pos, test_size=mask_prop
        )  # select which is masked for train
...
def get_mask(deleted_column_batch_data, flag_tensor=True):
    if flag_tensor == True:
        return (torch.abs(deleted_column_batch_data) > 0.0).float()
    else:
        return (np.abs(deleted_column_batch_data) > 0.0).astype(float)
...
def Test_UCI_batch(
    ...
    for counter in range(split + 1):
        ...
        if flag_model == "PNP_BNN":
            pred_mean, _ = PNP_copy.completion(
                data_input, mask_test_input, sigma_out_scale, size=size
            )
            pred_mean = pred_mean * mask_test_target
            _, pred_tot_ll = PNP_copy.test_log_likelihood(
                data_input, data_target, mask_test_input, sigma_out_scale, size=size
            )
        elif flag_model == "PNP":
            sigma_out = sigma_out_scale * torch.ones(1, test_input.shape[1])
            pred_mean = (
                PNP_copy.completion(data_input, mask_test_input, sigma_out)
                * mask_test_target
            )
            _, pred_tot_ll = PNP_copy.test_log_likelihood(
                data_input, data_target, mask_test_input, sigma_out
            )
            ...
            _, pred_tot_ll = PNP_copy.test_log_likelihood(
                ...
                X_test=data_target,
                W_sample=W_sample,
                mask=mask_test_input,
                sigma_out=sigma_out_scale,
                size=size,
            )
        else:
    ...
    RMSE = torch.sqrt(1.0 / (torch.sum(total_mask_target)) * total_se)
    # RMSE = torch.tensor(RMSE.data)
    RMSE = RMSE.clone().detach()
    # MAE = torch.tensor((1. / torch.sum(total_mask_target) * total_MAE).data)
    MAE = (1.0 / torch.sum(total_mask_target) * total_MAE).clone().detach()
    # NLL=torch.tensor(1. / (torch.sum(total_mask_target)) *total_NLL)
    NLL = (1.0 / (torch.sum(total_mask_target)) * total_NLL).clone().detach()
    return RMSE, MAE, NLL
...
def test_UCI_AL(
    ...
    **kwargs,
    ...
    if model_name == "PNP_SGHMC":
        ...
        RMSE_test, MAE_test, NLL_test = Test_UCI_batch(
            model.model,
            test_input,
            test_target,
    ...
    for num_selected in range(max_selection):
        ...
        if search == "Target" and num_selected > -1:
            if model_name == "PNP_BNN":
                test_input, index_array, test_pool = (
                    model.base_active_learning_z_target_BNN(
                        active_sample_number=sample_x,
                        test_input=test_input,
                        pool_data_tensor=test_pool,
                        target_data_tensor=test_target,
                    )
                )
            elif model_name == "PNP_SGHMC":
                test_input, index_array, test_pool = model.active_learn_target_test(
                    flag_same_pool=True,
                    test_input=test_input,
                    pool_data_tensor=test_pool,
                    target_data_tensor=test_target,
            ...
            if model_name == "PNP_BNN":
                ...
                test_input, index_array, test_pool = model.random_select_test(
                    test_input=test_input, pool_data_tensor=test_pool
                )
            else:
                raise NotImplementedError
        else:
...
def get_choice(observed_train_data):
    num_selected = torch.sum(torch.abs(observed_train_data) > 0.0, dim=0)  # obs_dim
...
def reduce_size(samples, perm1, perm2, flag_one_row=False):
    ...
    if flag_one_row == True:
        ...
        if len(samples.shape) == 4:
            ...
            sample_reduce_2 = sample_reduce_1[
                :, perm2, :, :, :
            ]  # reduce x reduce x Np x N x obs
...
def assign_zero_row_2D_with_target_reverse(data, data_target, value=0.0):
...
def BALD_Select_Explore(BALD_weight, step):
    ...
    active_entry_num = torch.sum(BALD_weight > 0.0)

    if active_entry_num == 0.0:  # Every thing is zero, means no way to draw it
        flag_full = True
        idx = []
        select_num = 0
...
def assign_zero_row_2D_with_target(data, data_target, value=0.0):
    data = torch.tensor(data.data)
    sum_data = torch.sum(torch.abs(data_target), dim=1)
    data[sum_data <= 0.001, :] = value
    return data  # Can be seen as the entries of data matrix with the positions of the unobserved rows of the data_target are 0
...
class MaskLayer2d(nn.Module):
    ...
    def forward(self, x, m):
        ...
        if len(m.shape) == 2:
            ...
            raise ValueError(
                f"cannot determine how to reshape mask with shape = {m.shape}"
            )
