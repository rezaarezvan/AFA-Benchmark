...
from afa_generative.afa_methods import EDDI, UniformSampler, IterativeSelector, SGHMC, base_Active_Learning_SGHMC_Decoder
from afa_generative.utils import ReadYAML, test_UCI_AL, Compute_AUIC_1D, remove_zero_row_2D, MaskLayer
from afa_generative.models import PVAE, Point_Net_Plus_BNN_SGHMC, fc_Net
from afa_generative.datasets import load_spam, load_diabetes, load_miniboone, data_split, get_xy, Boston
...
if __name__ == '__main__':
    ...
    if args.method == 'eddi':
        ...
        train_dataset, val_dataset, test_dataset = data_split(dataset, val_portion=0.2, test_portion=0.2)
        # Prepare dataloaders.
    ...
    for trial in range(args.num_trials):
        ...
        if args.method == 'eddi':
            ...
            counter_loop=0
            counter_selection = 0
            PNP_SGHMC = Point_Net_Plus_BNN_SGHMC(latent_dim=cfg['BNN_Settings']['latent_dim'], obs_dim=obs_dim, dim_before_agg=cfg['BNN_Settings']['dim_before_agg'],
            ...
            Adam_embedding = torch.optim.Adam(
                [PNP_SGHMC.encode_embedding, PNP_SGHMC.encode_bias], lr=cfg['Optimizer']['lr_sghmc'],
                betas=(cfg['Optimizer']['beta1'], cfg['Optimizer']['beta2']),
                weight_decay=cfg['Optimizer']['weight_decay'])
            ...
            Active_BALD = base_Active_Learning_SGHMC_Decoder(
                model=PNP_SGHMC,Infer_model=Infer_SGHMC,overall_data=dataset.Data_mat,rs=42,
                sigma_out=sigma_out,Optim_settings=cfg['Optimizer'],Adam_encoder=Adam_encoder,
                Adam_embedding=Adam_embedding,flag_clear_target_train=flag_clear_target_train,
                flag_clear_target_test=flag_clear_target_test,model_name='SGHMC Active')
            ...
            # Pretrain
            W_sample = Active_BALD.train_BNN(pretrain_data, eps=step_sghmc, max_sample_size=40, tot_epoch=tot_epoch+500,
                                             thinning=10,hyper_param_update=25000, sample_int=10,
                                             ...
                                             conditional_coef=conditional_coef_sghmc,target_dim=-1, 
            ...
            while counter_selection < max_selection:
                ...
                observed_train_BALD, train_pool_data_BALD, flag_full, num_selected = Active_BALD.base_active_learning_decoder(
                    balance_prop=balance_coef, coef=BALD_coef,observed_train=observed_train_BALD, 
                    pool_data=train_pool_data_BALD, step=num_selected_variable,flag_initial=flag_init_train, 
                    sigma_out=sigma_out, W_sample=W_sample,strategy='Opposite_Alpha',strategy_alternating=flag_weight,
                    Select_Split=True,selection_scheme=selection_scheme,idx_start=idx_start,idx_end=idx_end)
                ...
                # Random Seed?
                Active_BALD = base_Active_Learning_SGHMC_Decoder(
                    model=PNP_SGHMC, Infer_model=Infer_SGHMC,overall_data=dataset.Data_mat, 
                    rs=42, sigma_out=sigma_out,Optim_settings=cfg['Optimizer'], Adam_encoder=Adam_encoder,
                    Adam_embedding=Adam_embedding, flag_clear_target_train=flag_clear_target_train,
                    flag_clear_target_test=flag_clear_target_test,model_name='SGHMC Active')

                # Train the model
                W_sample = Active_BALD.train_BNN(observed_train_BALD_rz, eps=step_sghmc, max_sample_size=40,tot_epoch=epoch_orig+500,
                                                 thinning=10,hyper_param_update=25000,sample_int=10,
                                                 flag_hybrid=flag_hybrid, Adam_encoder=Adam_encoder,
