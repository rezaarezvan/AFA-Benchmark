...
class EDDI(nn.Module):
    ...
    def forward_multiple(self, x, num_features_list, verbose=False):
        ...
        Args:
          ...
          verbose:
        """
        for num, x_masked, _ in self.select_features_multiple(
            x, num_features_list, verbose
        ):
            ...
            for k in range(max_features):
                ...
                for j in range(num_features):
                    if m[i][j] == 1:
                        ...
                        if isinstance(mask_layer, MaskLayerGrouped):
                            inds = (
                                torch.where(mask_layer.group_matrix[j])[0]
                                .cpu()
                                .data.numpy()
                            )
                            original = x_row[:, inds]
    ...
    def evaluate(self, loader, max_features, metric):
        """
    ...
    def evaluate_multiple(self, loader, num_features_list, metric):
        """
...
def calculate_criterion(preds, task):
    ...
    if task == "regression":
        ...
        raise ValueError(
            f"unsupported task: {task}. Must be classification or regression"
        )


class Ma2018AFAMethod(AFAMethod):
    def __init__(
        self, sampler, predictor, task="classification", device=torch.device("cpu")
    ):
        super().__init__()
        assert hasattr(sampler, "impute")
        self.sampler = sampler
...
class IterativeSelector(nn.Module):
    ...
    def forward_multiple(self, x, num_features_list, num_samples=128, verbose=False):
        ...
        """
        for num, x_masked, _ in self.select_features_multiple(
            x, num_features_list, num_samples, verbose
        ):
            yield num, self.model(x_masked)
    ...
    def select_features_multiple(
        self, x, num_features_list, num_samples=128, verbose=False
    ):
        """
        Select features for multiple budgets.
    ...
    def evaluate(self, loader, max_features, metric, num_samples=128):
        """
        Evaluate mean performance across a dataset.
    ...
    def evaluate_multiple(self, loader, num_features_list, metric, num_samples=128):
        """
        Evaluate mean performance across a dataset.
        ...
        with torch.no_grad():
            for x, y in loader:
                ...
                # Calculate loss.
                for num, pred in self.forward_multiple(
                    x, num_features_list, num_samples
                ):
                    pred_dict[num].append(pred.cpu())
                label_list.append(y.cpu())
...
class base_Infer(object):
    ...
    def compute_pos_W(self, **kwargs):
        raise NotImplementedError(
            "Must override this method for last layer Gaussian model!"
        )
...
class SGHMC(base_Infer):
    def __init__(self, model, Infer_name="SGHMC"):
        ...
        self.V, self.M, self.r = (
            ...
            None,
            None,
        )  # This is reserved for train_SGHMC() Method
    ...
    def _SGHMC_step(self, **kwargs):
        ...
        for key_r, value_r in r.items():
            ...
            Diff_CB = torch.clamp((C_ - B_), min=0.0)

            if flag_optimizer:
                V_.data = 0.99 * V_.data + 0.01 * (G_.data**2)
                M_.data = 1.0 / torch.sqrt((torch.sqrt(V_.data)))
                C_ = 0.1 / math.sqrt(eps)
                B_ = 0.5 * math.sqrt(eps)
                Diff_CB = C_ - B_
                rescaled_noise = (
                    torch.sqrt(2 * Diff_CB * math.sqrt(eps) * eps * M_.data**2) * noise
                )
                value_r.data = (
                    value_r.data
                    - eps * (M_.data**2) * G_.data
                    - 0.1 * value_r.data
                    + update_noise * rescaled_noise
                )
                ...
                if flag_compensator:
                    M_old_ = M_old[key_r]
                    Gamma = (M_.data - M_old_.data) / (
                        torch.sign(value_r.data)
                        * torch.clamp(torch.abs(value_r.data), min=1e-7)
                    )
                rescaled_noise = (
                    torch.sqrt(2 * Diff_CB * math.sqrt(eps2) * eps2 * M_.data**2)
                    * noise
                )
                if flag_compensator:
                    value_r.data = (
                        value_r.data
                        - eps2 * (M_.data**2) * G_.data
                        - 0.1 * value_r.data
                        + 1 * rescaled_noise
                    )
                else:
    ...
    def _SGHMC_burnin_step(self, **kwargs):
        ...
        for key_r, value_r in r.items():
            ...
            V_ = V[key_r]
            # Update g and V
            g_.data = g_.data - 1.0 / tau_ * g_.data + 1.0 / tau_ * G_.data
            V_.data = torch.clamp(
                V_.data - 1.0 / tau_ * V_.data + 1.0 / (tau_) * ((G_.data) ** 2),
                min=0.0001,
            )
            if flag_update_tau:
                # Update window
        ...
        for key_r, value_r in r.items():
            G_ = G_dict[key_r]
            V_ = V[key_r]
            M_ = 1.0 / torch.sqrt(V_)  # equivalent to M^-1
            M[key_r] = M_
            W_ = W_dict[key_r]
            ...
            rescaled_noise = (
                0
                * torch.sqrt(torch.clamp(2 * (eps**3) / (V_) * C_ - eps**4, min=0.0))
                * noise
            )  # 1*torch.sqrt(2*eps*Diff_CB) *noise
    ...
    def _init_mat(self):
        ...
        if not self.flag_only_output_layer:
            for layer_ind in range(self.model.decoder.hidden_layer_num):
                V["weight_layer_%s" % (layer_ind)] = torch.ones(
                    W_dict["weight_layer_%s" % (layer_ind)].shape
                )
                V["bias_layer_%s" % (layer_ind)] = torch.ones(
                    W_dict["bias_layer_%s" % (layer_ind)].shape
                )
                M["weight_layer_%s" % (layer_ind)] = 1.0 / torch.sqrt(
                    V["weight_layer_%s" % (layer_ind)]
                )
                M["bias_layer_%s" % (layer_ind)] = 1.0 / (
                    torch.sqrt(V["bias_layer_%s" % (layer_ind)])
                )
                r["weight_layer_%s" % (layer_ind)] = 0.01 * torch.randn(
                    W_dict["weight_layer_%s" % (layer_ind)].shape
                )
                r["bias_layer_%s" % (layer_ind)] = 0.01 * torch.randn(
                    W_dict["bias_layer_%s" % (layer_ind)].shape
                )
                tau["weight_layer_%s" % (layer_ind)] = 1.01 * torch.ones(
                    W_dict["weight_layer_%s" % (layer_ind)].shape
                )
                tau["bias_layer_%s" % (layer_ind)] = 1.01 * torch.ones(
                    W_dict["bias_layer_%s" % (layer_ind)].shape
                )
        # Output layer
        V["weight_out"] = torch.ones(W_dict["weight_out"].shape)
        V["bias_out"] = torch.ones(W_dict["bias_out"].shape)
        M["weight_out"] = 1.0 / torch.sqrt(V["weight_out"])
        M["bias_out"] = 1.0 / (torch.sqrt(V["bias_out"]))
        ...
        if self.flag_LV:
            ...
            M["weight_out_LV"] = 1.0 / torch.sqrt(V["weight_out_LV"])
            M["bias_out_LV"] = 1.0 / (torch.sqrt(V["bias_out_LV"]))
    ...
    def sample_pos_W(
        ...
        **kwargs,
        ...
        observed_train_size = observed_train.shape[
            0
        ]  # Assume the observed_train is after removing 0 row
        batch_size = int(observed_train_size / 2.0)
        if batch_size > 100:
            batch_size = 100
        # Define Data loader
        ...
        while iter <= tot_iter:
            for idx, data in enumerate(train_loader):
                ...
                mask_drop = np.array(
                    [bernoulli.rvs(1 - Drop_p_var, size=data.shape[1])] * data.shape[0]
                )
                ...
                if flag_hybrid:
                    ...
                    mask_drop_hybrid[:, target_dim] = 1.0  # Reserve the target dim
                ...
                # Compute ELBO
                if flag_hybrid:
                    ELBO, ELBO_mean = self._compute_target_ELBO(
                        data,
                        mask,
                        mask_target_ELBO,
                        observed_train_size,
                        ...
                        W_mean_prior=0.0,
                        W_sigma_prior=1.0,
                        record_memory=flag_record,
                        target_dim=target_dim,
                        conditional_coef=conditional_coef,
                    )
                else:
                    ELBO, ELBO_mean = self._compute_ELBO(
                        data,
                        mask,
                        observed_train_size,
                        W_dict,
                ...
                if not flag_burnin:
                    ...
                    W_dict, r, M, V, tau, g = self._SGHMC_burnin_step(
                        W_dict=W_dict,
                        G_dict=G_dict,
                        eps=eps,
                        M=M,
    ...
    def _compute_ELBO(
        ...
        obs_dim = X.shape[1]
        # Sample Z_i
        z, encoding = self.model.sample_latent_variable(
            X, mask, size=size_Z
        )  # N_z x N x latent_dim
        ...
        if self.model.flag_log_q == True:
            ...
            q_sigma = torch.clamp(
                torch.sqrt(
                    torch.clamp((encoding[:, self.model.latent_dim :]) ** 2, min=1e-8)
                ),
                min=1e-4,
                max=30,
            )
        ...
        if self.flag_LV:
            ...
            # Log likelihood
            log_likelihood = self._scaled_gaussian_log_likelihood(
                X, mask, decode_X, sigma_out, sigma_out_target_scale=1.0
            )  # N_z x N
            ELBO = log_likelihood - 1.0 * torch.unsqueeze(
                self.model._KL_encoder(q_mean, q_sigma, z_sigma_prior), dim=0
            )  # N_z x N
            ELBO = torch.sum(
                torch.mean(ELBO, dim=0)
            ) / num_X * data_N + 1 * self._compute_W_prior(
                W_mean_prior=W_mean_prior, W_sigma_prior=W_sigma_prior
            )  # TODO: Need to check if we only have prior, does it have gradient? Yes!
            ELBO_mean = ELBO / data_N
        return ELBO, ELBO_mean

    def _compute_target_ELBO(
        self,
        X,
        mask,
        mask_target_ELBO,
        ...
        Xdy[:, target_dim] = 0.0  # zero the target dim
        # mask_Xdy = torch.tensor(mask_target_ELBO.data)
        mask_Xdy = mask_target_ELBO.clone().detach()
        mask_Xdy[:, target_dim] = 0.0

        z_XUy, encoding_XUy = self.model.sample_latent_variable(
            XUy, mask_target_ELBO, size=size_Z
        )  # N_z x N x latent_dim
        z_Xdy, encoding_Xdy = self.model.sample_latent_variable(
            Xdy, mask_Xdy, size=size_Z
        )  # N_z x N x latent_dim

        z_X, encoding_X = self.model.sample_latent_variable(
            X, mask, size=size_Z
        )  # N_z x N x latent_dim
        ...
        # q_sigma=F.softplus(encoding[:,self.latent_dim:]) # N x latent_dim
        if self.model.flag_log_q == True:
            q_sigma_XUy = torch.sqrt(
                torch.exp(encoding_XUy[:, self.model.latent_dim :])
            )
            q_sigma_Xdy = torch.sqrt(
                torch.exp(encoding_Xdy[:, self.model.latent_dim :])
            )
            q_sigma_X = torch.sqrt(torch.exp(encoding_X[:, self.model.latent_dim :]))

        else:
            q_sigma_XUy = torch.clamp(
                torch.sqrt((encoding_XUy[:, self.model.latent_dim :]) ** 2),
                min=1e-5,
                max=30.0,
            )
            q_sigma_Xdy = torch.clamp(
                torch.sqrt((encoding_Xdy[:, self.model.latent_dim :]) ** 2),
                min=1e-5,
                max=30.0,
            )
            q_sigma_X = torch.clamp(
                torch.sqrt((encoding_X[:, self.model.latent_dim :]) ** 2),
                min=1e-5,
                max=30.0,
            )
        ...
        if not record_memory:
            ...
            q_mean_XUy, q_sigma_XUy, q_sigma_X = (
                q_mean_XUy.clone().detach(),
                q_sigma_XUy.clone().detach(),
                q_sigma_X.clone().detach(),
            )
            q_mean_Xdy, q_sigma_Xdy, q_mean_X = (
                q_mean_Xdy.clone().detach(),
                q_sigma_Xdy.clone().detach(),
                q_mean_X.clone().detach(),
            )

        q_mean_Xdy = q_mean_Xdy.clone().detach()
        q_sigma_Xdy = q_sigma_Xdy.clone().detach()

        KL_z_target = self.model._KL_encoder_target_ELBO(
            q_mean_XUy, q_sigma_XUy, q_mean_Xdy, q_sigma_Xdy
        )  # N
        ...
        if self.flag_LV:
            ...
            # Log likelihood
            log_likelihood_recon = self._scaled_gaussian_log_likelihood(
                X, mask, decode_X, sigma_out, flag_target=False
            )  # N_z x N
            ELBO_recon = 1 * log_likelihood_recon - 1 * torch.unsqueeze(
                KL_z, dim=0
            )  # N_z x N
            ELBO_recon = torch.sum(
                torch.mean(ELBO_recon, dim=0)
            ) / num_X * data_N + 1 * self._compute_W_prior(
                W_mean_prior=W_mean_prior, W_sigma_prior=W_sigma_prior
            )  # TODO: Need to check if we only have prior, does it have gradient? Yes!
        ...
        mask_y[:, target_dim] = 1.0  # Only reserve the target dim and disable all other
        ...
        if self.flag_LV:
            ...
            log_likelihood_target = self._scaled_gaussian_log_likelihood(
                XUy, mask_y, decode_target, sigma_out, flag_target=True
            )  # N_z x N
            ELBO_target = log_likelihood_target - 1.0 * torch.unsqueeze(
                KL_z_target, dim=0
            )  # N_z x N
            ELBO_target = torch.sum(
                torch.mean(ELBO_target, dim=0)
            ) / num_X * data_N + 1 * self._compute_W_prior(
                W_mean_prior=W_mean_prior, W_sigma_prior=W_sigma_prior
            )
    ...
    def _scaled_gaussian_log_likelihood(
        self,
        X,
        mask,
        decode,
        sigma_out,
        sigma_out_target_scale=1.0,
        flag_target=False,
        **kwargs,
    ):
        if self.flag_LV:
        ...
        active_dim_scale = torch.clamp(
            torch.sum(torch.abs(get_mask(X)) > 0.0, dim=1).float(), min=1
        )  # N
        X = X * mask
        ...
        if decoding_size == 3:  # Nz x N x obs_dim
            ...
            if flag_target == False:
                active_dim = torch.clamp(
                    torch.sum(torch.abs(mask) > 0.0, dim=1).float(), min=1
                )  # N
            else:
                active_dim = 1.0
            ...
            if self.flag_LV:
                ...
                log_likelihood = (
                    -0.5
                    * torch.sum((X_expand_X - decoding_X) ** 2 / (sigma_out**2), dim=2)
                    - 0.5
                    * torch.sum(
                        (X_expand_Y - decoding_Y) ** 2
                        / ((sigma_out * sigma_out_target_scale) ** 2),
                        dim=2,
                    )
                    - 0.5
                    * torch.unsqueeze(torch.sum(mask, dim=1), dim=0)
                    * (math.log(sigma_out**2) + math.log(2 * np.pi))
                )  # N_z x N
            ...
            if flag_target == False:
                ...
                log_likelihood = (
                    log_likelihood / active_dim_expand * active_dim_scale_expand
                )  # N_z x N
            else:
                active_dim_scale_expand = torch.unsqueeze(active_dim_scale, dim=0)
    ...
    def _compute_W_prior(self, W_mean_prior=0.0, W_sigma_prior=1.0):
        flat_weight, out_wight, out_bias = (
            self.model.decoder._flatten_stat()
        )  # 1 x D_weight,N_in x N_out and N_out
        if self.model.decoder.flag_only_output_layer == False:
            # Prior of flat_weight
            log_prior_shared = torch.sum(
                -0.5 / (W_sigma_prior**2) * ((flat_weight - W_mean_prior) ** 2)
                - 0.5 * math.log(W_sigma_prior**2)
                - 0.5 * math.log(2 * np.pi)
            )
            # Compute out_weight prior
            log_prior_out_weight = torch.sum(
                -0.5 / (W_sigma_prior**2) * ((out_wight - W_mean_prior) ** 2)
                - 0.5 * math.log(W_sigma_prior**2)
                - 0.5 * math.log(2 * np.pi)
            )
            log_prior_out_bias = torch.sum(
                -0.5 / (W_sigma_prior**2) * ((out_bias - W_mean_prior) ** 2)
                - 0.5 * math.log(W_sigma_prior**2)
                - 0.5 * math.log(2 * np.pi)
            )
        ...
        if self.flag_LV:
            ...
            with torch.no_grad():
                complete = self.completion(
                    X_in, mask, W_sample, size_Z=size, record_memory=False
                )
            complete = complete.clone().detach()
            test_mask = get_mask(X_test)

            X_expand = torch.unsqueeze(
                torch.unsqueeze(X_test, dim=0), dim=0
            )  # 1 x 1 x N x obs_dim
            mask_expand = torch.unsqueeze(
                torch.unsqueeze(test_mask, dim=0), dim=0
            )  # 1 x 1 x N x obs_dim
            decoding = complete * mask_expand  # N_w x N_z x N x obs_dim
            with torch.no_grad():
                # Proper NLL
                log_likelihood = -0.5 * torch.sum(
                    (X_expand - decoding) ** 2 / (sigma_out**2), dim=3
                ) - 0.5 * torch.unsqueeze(
                    torch.unsqueeze(torch.sum(test_mask, dim=1), dim=0), dim=0
                ) * (math.log(sigma_out**2) + math.log(2 * np.pi))  # N_w x N_z x N

                log_likelihood = log_likelihood.view(
                    -1, log_likelihood.shape[2]
                )  # (N_w x N_z) x N
                pred_log_likelihood = torch.logsumexp(log_likelihood, dim=0) - math.log(
                    float(log_likelihood.shape[0])
                )  # N
                mean_pred_log_likelihood = (
                    1.0 / (torch.sum(test_mask)) * torch.sum(pred_log_likelihood)
                )
...
class base_Active_Learning_SGHMC(object):
    def __init__(
        ...
        Adam_encoder,
        Adam_embedding,
        flag_clear_target_train,
        flag_clear_target_test,
        rs=10,
    ...
    def _get_pretrain_data(self, target_dim=-1, **kwargs):
        ...
        _, pretrain_data, _, _ = train_test_split(
            self.train_data,
            self.train_data,
            test_size=pretrain_data_number,
            random_state=rs,
            shuffle=False,
        )

        self.train_data[self.train_data.shape[0] - pretrain_data.shape[0] :, :] = 0.0
        self.init_observed_train = np.zeros(self.train_data.shape)  # Original size
        self.init_observed_train[
            self.train_data.shape[0] - pretrain_data.shape[0] :, :
        ] = pretrain_data
        ...
        self.init_observed_train_tensor = (
            torch.from_numpy(self.init_observed_train).float().cuda()
        )
        self.train_pool_tensor = self.train_data_tensor.clone().detach()
        if self.flag_clear_target_train:
            self.train_pool_tensor[:, target_dim] = 0.0  # Remove the target dim
    ...
    def _data_preprocess(self, target_dim=-1, test_missing=0.2, **kwargs):
        ...
        train_data, valid_data, _, _ = train_test_split(
            train_data, train_data, test_size=0.1, random_state=rs, shuffle=True
        )

        # Mask the train data
        train_mask, train_mask_other = base_generate_mask_incomplete(
            train_data, mask_prop=missing_prop + 1e-5
        )
        valid_mask, valid_mask_other = base_generate_mask_incomplete(
            valid_data, mask_prop=missing_prop + 1e-5
        )

        if self.flag_clear_target_train == True:  # Reserve the target variable
            train_mask[:, target_dim] = 1.0
            train_mask_other[:, target_dim] = 0.0
            valid_mask[:, target_dim] = 1.0
            valid_mask_other[:, target_dim] = 0.0
            valid_mask_input = np.ones(valid_mask.shape)
            valid_mask_input[:, target_dim] = 0.0
            valid_mask_target = np.zeros(valid_mask.shape)
            valid_mask_target[:, target_dim] = 1.0
        else:
        ...
        if self.flag_clear_target_test == True:
            ...
            # NO CLEAR TARGET
            test_mask, target_mask = base_generate_mask_incomplete(
                test_data, mask_prop=missing_prop + test_missing
            )
            _, valid_mask_target = base_generate_mask_incomplete(
                valid_data, mask_prop=missing_prop + test_missing
            )
        ...
        self.valid_data_target_tensor = (
            torch.from_numpy(self.valid_data_target).float().cuda()
        )
        self.valid_data_input_tensor = (
            torch.from_numpy(self.valid_data_input).float().cuda()
        )
        ...
        if self.flag_clear_target_train == True:
            self.train_pool_tensor[:, target_dim] = 0.0  # Remove the target variable
        ...
        **kwargs,
        ...
        if flag_reset_optim == True:
            ...
            Adam_encoder = torch.optim.Adam(
                list(self.model.encoder_before_agg.parameters())
                + list(self.model.encoder_after_agg.parameters()),
            ...
            Adam_embedding = torch.optim.Adam(
                [self.model.encode_embedding, self.model.encode_bias],
                lr=self.Optim_settings["lr_sghmc"],
                betas=(self.Optim_settings["beta1"], self.Optim_settings["beta2"]),
                weight_decay=self.Optim_settings["weight_decay"],
            )
        ...
        W_sample, _, _, _ = train_SGHMC(
            ...
            tot_epoch=tot_epoch,
            thinning=thinning,
            hyper_param_update=hyper_param_update,
    ...
    def random_select_test(self, **kwargs):
        ...
        test_input = kwargs[
            "test_input"
        ]  # If this is the first time run, this should be all initialized at 0
        ...
        for user_i in range(total_user):
            non_zero_idx = (
                torch.abs(pool_data_tensor[user_i, :]) > 0.0
            ).nonzero()  # 1D idx tensor

            select_idx = non_zero_idx[
                torch.randperm(len(non_zero_idx))[0]
            ]  # random select 1 index
            test_input[user_i, select_idx] = pool_data_tensor[user_i, select_idx]
            # remove the pool
            pool_data_tensor[user_i, select_idx] = 0.0
            idx_array.append(select_idx)
        return test_input, idx_array, pool_data_tensor

    def generate_active_learn_input_z_target_test(
        self,
        test_input,
        pool_candidate,
        target_candidate,
        flag_only_i=False,
        flag_same_pool=False,
        **kwargs,
    ):
        ...
        if flag_only_i == False:
            ...
            # Generate X_phi, X_o
            target_candidate_reshape = target_candidate.view(
                -1, target_candidate.shape[2], target_candidate.shape[3]
            )  # tot x N x obs
            target_candidate_reshape = target_candidate_reshape.view(
                target_candidate_reshape.shape[0], -1
            )  # tot x (N x obs)

            # Non-zero idx
            non_zero_idx = (
                torch.abs(target_candidate_reshape) > 0.0
            ).nonzero()  # 2D array
            ...
            non_zero_idx_expand = torch.unsqueeze(non_zero_idx, dim=0).repeat(
                total_sample_number, 1
            )  # tot x d_pool
            test_input_reshape = test_input.view(1, -1).repeat(
                total_sample_number, 1
            )  # tot x (Nxobs)

            # select index
            target_candidate_reshape_selected = torch.index_select(
                target_candidate_reshape, dim=1, index=non_zero_idx
            )  # TODO: Check (total) x d_target (Checked)

            active_input_o_target = test_input_reshape.scatter_(
                dim=1, index=non_zero_idx_expand, src=target_candidate_reshape_selected
            )  # tot x (Nxobs)
            active_input_o_target = active_input_o_target.view(
                total_sample_number, N, obs
            )  # TODO: Check tot x N x obs and Check if it does the desired behavior (Checked)

        if flag_same_pool == True:
            # Generate X_phi,X_o,X_i
            total_sample_number_pool = pool_candidate.shape[0] * pool_candidate.shape[1]
            d_active = torch.sum(torch.abs(pool_candidate[0, 0, 0, :]) > 0.0).int()
            pool_candidate_reshape = pool_candidate.view(-1, N, obs)  # tot x N x obs
            # print('d_active:%s'%(d_active))
            active_input_o_target_rep = torch.unsqueeze(
                active_input_o_target, dim=2
            ).repeat(1, 1, d_active, 1)  # TODO: Check if tot x N x d x obs

            non_zero_idx_pool = (
                torch.abs(pool_candidate_reshape) > 0.0
            ).nonzero()  # 3 eleement matrix
            non_zero_idx_pool = non_zero_idx_pool[:, 1:]  # 2D with N,pool_dim
            non_zero_idx_pool_reshape = non_zero_idx_pool[0 : N * d_active, 1:2].view(
                N, d_active
            )  # TODO: Check if N x d (Checked)
            non_zero_idx_pool_reshape_final = torch.unsqueeze(
                torch.unsqueeze(non_zero_idx_pool_reshape, dim=0), dim=3
            ).repeat(
                total_sample_number_pool, 1, 1, 1
            )  # TODO: Check if tot x N x d x 1

            pool_candidate_reshape_reshape = pool_candidate_reshape.view(
                total_sample_number_pool, -1
            )  # tot x (N x obs)
            non_zero_idx_tmp = (
                torch.abs(pool_candidate_reshape_reshape) > 0.0
            ).nonzero()
            non_zero_idx_tmp = torch.unique(non_zero_idx_tmp[:, 1])  # 1 D array

            pool_candidate_reshape_reshape = torch.index_select(
                pool_candidate_reshape_reshape, dim=1, index=non_zero_idx_tmp
            )  # TODO: Check tot x (N x d)
            pool_candidate_reshape_reshape = pool_candidate_reshape_reshape.view(
                -1, N, d_active
            )
            pool_candidate_reshape_reshape = torch.unsqueeze(
                pool_candidate_reshape_reshape, dim=3
            )  # TODO: Check if tot x N x d x 1(Checked)
            active_input_o_target_i = active_input_o_target_rep.scatter_(
                dim=3,
                index=non_zero_idx_pool_reshape_final,
                src=pool_candidate_reshape_reshape,
            )  # TODO Check if does desired and shape tot x N x d x obs
            return (
                active_input_o_target,
                active_input_o_target_i,
                non_zero_idx_pool_reshape,
            )
        else:
            active_input_o_target_slice = active_input_o_target[:, slice, :]
            pool_candidate_slice = pool_candidate.view(
                -1, pool_candidate.shape[2], pool_candidate.shape[3]
            )  # (N_zxN_W) x N x obs_dim
            pool_candidate_slice = torch.unsqueeze(
                pool_candidate_slice[:, slice, :], dim=2
            )  # (total_sample_num) x obs_dim x 1
            non_zero_idx = (
                torch.abs(pool_candidate_slice) > 0.0
            ).nonzero()  # 3D tensor
            ...
            # Non zero pool
            pool_candidate_slice = torch.index_select(
                pool_candidate_slice, dim=1, index=non_zero_idx_dim
            )  # (total) x d_pool x 1
            # index array
            ...
            active_input_pool_target = torch.unsqueeze(
                active_input_o_target_slice, dim=1
            ).repeat(1, total_pool_size, 1)  # tot x d x obs_dim

            active_input_pool_target = active_input_pool_target.scatter_(
                dim=2, index=non_zero_idx_array, src=pool_candidate_slice
            )  # total x d x obs_dim
            return active_input_o_target, active_input_pool_target, non_zero_idx_dim

    def generate_active_learn_input_z_i_test(
        self, test_input, pool_candidate, flag_same_pool=False, **kwargs
    ):
        ...
        if flag_same_pool == True:
            ...
            test_input_reshape = torch.unsqueeze(test_input, dim=1).repeat(
                1, d_active, 1
            )  # N x d x obs
            test_input_reshape = torch.unsqueeze(test_input_reshape, dim=0).repeat(
                sample_tot, 1, 1, 1
            )  # tot x N x d x obs

            non_zero_idx = (torch.abs(pool_candidate_reshape) > 0.0).nonzero()
            non_zero_idx = non_zero_idx[:, 1:]  # 2 D array
            non_zero_idx_reshape = non_zero_idx[0 : N * d_active, 1:2].view(
                N, d_active
            )  # N x d_active
            non_zero_idx_reshape_final = torch.unsqueeze(
                torch.unsqueeze(non_zero_idx_reshape, dim=2), dim=0
            ).repeat(sample_tot, 1, 1, 1)  # tot x N x d x 1

            pool_candidate_flat = pool_candidate_reshape.view(sample_tot, -1)
            non_zero_idx_tmp = (torch.abs(pool_candidate_flat) > 0.0).nonzero()
            non_zero_idx_tmp = torch.unique(non_zero_idx_tmp[:, 1])  # 1 d array

            pool_candidate_select = torch.index_select(
                pool_candidate_flat, dim=1, index=non_zero_idx_tmp
            )
            pool_candidate_select = torch.unsqueeze(
                pool_candidate_select.view(sample_tot, N, d_active), dim=3
            )  # tot x N x d x 1

            active_input_o_i = test_input_reshape.scatter_(
                dim=3, index=non_zero_idx_reshape_final, src=pool_candidate_select
            )
            ...
            test_input_slice = torch.unsqueeze(
                torch.unsqueeze(test_input_slice, dim=0), dim=0
            ).repeat(total_sample_number, total_pool_size, 1)

            active_input = test_input_slice.scatter_(
                dim=2, index=non_zero_idx_array, src=pool_candidate_slice
            )  # total x d x obs_dim
    ...
    def active_learn_target_test(self, **kwargs):
        ...
        if split > 1 and split < test_input.shape[0]:
            ...
            for idx in range(split + 1):
                ...
                z, _ = self.model.sample_latent_variable(
                    data_input, data_input_mask, size=size_z
                )  # size_z x N x latent
                if self.Infer_model.flag_LV:
                    decode, _ = self.Infer_model.sample_X(z, W_sample)
                else:
                    decode = self.Infer_model.sample_X(
                        z, W_sample
                    )  # N_w x Nz x N x obs
                ...
                target_mask_expand = torch.unsqueeze(
                    torch.unsqueeze(target_mask, dim=0), dim=0
                )  # 1 x 1 x N x obs_dim

                target_candidate = (
                    target_mask_expand * decode
                )  # N_w x N_z x N x obs_dim

                pool_mask = get_mask(data_pool)
                pool_mask_expand = torch.unsqueeze(
                    torch.unsqueeze(pool_mask, dim=0), dim=0
                )  # 1 x 1 x N x obs_dim
                ...
                if flag_same_pool:
                    ...
                    active_input_o_target, active_input_o_i_target, non_zero_idx = (
                        self.generate_active_learn_input_z_target_test(
                            data_input,
                            pool_candidate,
                            target_candidate,
                            flag_only_i=False,
                    ...
                    active_input_o_i, non_zero_idx = (
                        self.generate_active_learn_input_z_i_test(
                            data_input, pool_candidate, flag_same_pool=True
                        )
                    )

                    mask_active_target = get_mask(
                        active_input_o_target
                    )  # tot x N x obs
                    mask_active_pool_target = get_mask(
                        active_input_o_i_target
                    )  # tot x N x d x obs_dim
                    mask_active = get_mask(active_input_o_i)  # tot x N x d x obs

                    encoding_target = self.model._encoding(
                        active_input_o_target, mask_active_target
                    )  # total x N x 2*latent
                    encoding_pool_target = self.model._encoding(
                        active_input_o_i_target, mask_active_pool_target
                    )  # tot x N x d x 2*latent
                    encoding_o_i = self.model._encoding(
                        active_input_o_i, mask_active
                    )  # total x N x d x 2*latent

                    encoding_o = self.model._encoding(
                        data_input, data_input_mask
                    )  # N x 2*latent
                    ...
                    encoding_target = torch.unsqueeze(encoding_target, dim=2).repeat(
                        1, 1, encoding_pool_target.shape[2], 1
                    )  # tot x N x d x 2*latent
                    KL_2 = self._gaussian_KL_all(
                        encoding_pool_target, encoding_target
                    )  # tot x N x d

                    encoding_o = torch.unsqueeze(encoding_o, dim=0).repeat(
                        encoding_o_i.shape[0], 1, 1
                    )
                    encoding_o = torch.unsqueeze(encoding_o, dim=2).repeat(
                        1, 1, encoding_o_i.shape[2], 1
                    )  # tot  x N x d x 2*latent
                    KL_1 = self._gaussian_KL_all(
                        encoding_o_i, encoding_o
                    )  # tot x N x d
                    ...
                    # original index
                    idx_selected = non_zero_idx[
                        np.arange(non_zero_idx.shape[0]), idx_max
                    ]  # N

                    # update data_input
                    test_input[torch.arange(pre_idx, idx), idx_selected] = (
                        pool_data_tensor[torch.arange(pre_idx, idx), idx_selected]
                    )
                    # update the pool by removing the selected ones
                    pool_data_tensor[torch.arange(pre_idx, idx), idx_selected] = 0.0

                else:
                    # less efficient computation, compute EDDI for single row at a time
                    active_input_o_target, _, _ = (
                        self.generate_active_learn_input_z_target_test(
                            data_input,
                            pool_candidate,
                            target_candidate,
                    ...
                    )  # what ever slice is fine

                    for user_i in range(total_user):
                        _, active_input_o_i_target, non_zero_idx = (
                            self.generate_active_learn_input_z_target_test(
                                data_input,
                                pool_candidate,
                                target_candidate,
                                flag_only_i=True,
                                flag_same_pool=False,
                                active_input_o_target=active_input_o_target,
                                slice=user_i,
                            )
                        )
                        active_input_o_i, non_zero_idx = (
                        ...
                        active_input_o_target_slice = active_input_o_target[
                            :, user_i, :
                        ]

                        mask_active_target = get_mask(
                            active_input_o_target_slice
                        )  # tot  x obs
                        mask_active_pool_target = get_mask(
                            active_input_o_i_target
                        )  # tot x d x obs_dim
                        ...
                        encoding_target = self.model._encoding(
                            active_input_o_target_slice, mask_active_target
                        )  # total x 2*latent
                        encoding_pool_target = self.model._encoding(
                            active_input_o_i_target, mask_active_pool_target
                        )  # tot  x d x 2*latent
                        encoding_o_i = self.model._encoding(
                            active_input_o_i, mask_active
                        )  # total  x d x 2*latent

                        encoding_o = self.model._encoding(
                            torch.unsqueeze(data_input[user_i, :], dim=0),
                            torch.unsqueeze(data_input_mask[user_i, :], dim=0),
                        )  # 1 x 2*latent
                        ...
                        encoding_target = torch.unsqueeze(
                            encoding_target, dim=1
                        ).repeat(
                            1, encoding_pool_target.shape[1], 1
                        )  # tot x d x 2*latent
                        KL_2 = self._gaussian_KL(
                            encoding_pool_target, encoding_target
                        )  # tot x d
                        ...
                        encoding_o = encoding_o.repeat(
                            1, encoding_o_i.shape[1], 1
                        )  # tot x d x 2*latent
                        ...
                        # update data_input
                        test_input[counter_idx, idx_selected] = pool_data_tensor[
                            counter_idx, idx_selected
                        ]
                        # update the pool by removing the selected ones
                        pool_data_tensor[counter_idx, idx_selected] = 0.0
                        counter_idx += 1
                pre_idx = idx
    ...
    def _gaussian_KL(self, encoding1, encoding2):
        ...
        mean1, mean2 = (
            encoding1[:, :, 0 : self.model.latent_dim],
            encoding2[:, :, 0 : self.model.latent_dim],
        )  # tot x d x latent
        if self.model.flag_log_q == True:
            sigma1, sigma2 = (
                torch.sqrt(torch.exp(encoding1[:, :, self.model.latent_dim :])),
                torch.sqrt(torch.exp(encoding2[:, :, self.model.latent_dim :])),
            )  # tot x d x latent
        else:
            sigma1, sigma2 = (
                torch.sqrt(encoding1[:, :, self.model.latent_dim :] ** 2),
                torch.sqrt(encoding2[:, :, self.model.latent_dim :] ** 2),
            )  # tot x d x latent
        KL = (
            torch.log(sigma2 / sigma1)
            + (sigma1**2 + (mean1 - mean2) ** 2) / (2 * sigma2**2)
            - 0.5
        )  # tot x d x latent
...
class base_Active_Learning_SGHMC_Decoder(base_Active_Learning_SGHMC):
    ...
    def get_target_variable(
        self, observed_train, observed_train_before, target_dim, train_data=None
    ):
        ...
        if train_data is None:
            observed_train[sum_diff > 0.0, target_dim] = self.train_data_tensor[
                sum_diff > 0.0, target_dim
            ]  # Assume train_data_tensor has the same arrangement as observed_train
        else:
            observed_train[sum_diff > 0.0, target_dim] = train_data[
                sum_diff > 0.0, target_dim
            ]
    ...
    def _H_Xid_Xo(self, decode, comp_mean, sigma_out, flag_reduce_size=False, **kwargs):
        ...
        if flag_reduce_size == True:
            perm1, perm2 = (
                torch.randperm(decode.shape[0])[0:10],
                torch.randperm(decode.shape[1])[0:5],
            )
        ...
        comp_mean_expand = torch.unsqueeze(comp_mean, dim=0).repeat(
            comp_mean.shape[0], 1, 1, 1
        )  # (NwNz) x (NwNz) x N x obs
        ...
        if self.Infer_model.flag_LV:
            ...
            log_likelihood = (
                -0.5 * math.log(2 * np.pi)
                - 0.5 * math.log(sigma_out**2)
                - 1.0 / (2 * sigma_out**2) * (decode_exapnd - comp_mean_expand) ** 2
            )  # (NwNz) x (NwNz) x N x obs
        # Compute logsumexp
        marginal_log_likelihood = torch.logsumexp(log_likelihood, dim=1) - math.log(
            float(decode_exapnd.shape[0])
        )  # (Nw_Nz) x N x obs
    ...
    def _H_X_id_W_Xo(
        self, decode, comp_mean, sigma_out, flag_reduce_size=False, **kwargs
    ):  # Compute H[p(X_id|X_o,W)]
        ...
        comp_mean_expand = torch.unsqueeze(comp_mean, dim=1).repeat(
            1, comp_mean.shape[1], 1, 1, 1
        )  # N_W x N_z x N_z x N x obs
        if self.Infer_model.flag_LV:
            raise RuntimeError("flag_LV must be False")
        else:
    ...
    def _H_X_Phi_O_id(
        self, decode, observed_data_row, W_sample, row_idx, target_dim=-1
    ):
        # Remove memory
        ...
        perm1, perm2 = (
            torch.randperm(decode.shape[0])[0:5],
            torch.randperm(decode.shape[1])[0:5],
        )
        decode = reduce_size(
            decode, perm1, perm2, flag_one_row=True
        )  # red_w x red_z  x obs
        decode = decode.view(-1, decode.shape[-1])  # tot x obs
        tot = decode.shape[0]
        # Get vacance location
        idx = (
            (torch.abs(observed_data_row[0:-1]) == 0).nonzero().view(-1)
        )  # N_id (get rid of target variable)
        if idx.shape[0] == 0:
            return 0 * torch.ones(observed_data_row.shape)
        size_idx = idx.shape[0]  # N_pool
        observed_data_row_dup = torch.unsqueeze(
            torch.unsqueeze(observed_data_row, dim=0), dim=0
        ).repeat(tot, size_idx, 1)  # tot x N_pool x obs
        idx_dup = torch.unsqueeze(
            torch.unsqueeze(idx, dim=0).repeat(tot, 1), dim=2
        )  # tot x N_pool x 1
        mask_candidate = 1 - get_mask(observed_data_row)
        mask_candidate[-1] = 0
        decode = decode * torch.unsqueeze(
            mask_candidate, dim=0
        )  # tot x obs with zero's
        ...
        # Sample X_phi
        observed_data_row_dup = observed_data_row_dup.view(
            -1, observed_data_row_dup.shape[2]
        )  # (totxN_pool) x obs
        # Sample z
        mask_observed_data_exp = get_mask(observed_data_row_dup)
        z, _ = self.model.sample_latent_variable(
            observed_data_row_dup, mask_observed_data_exp, size=10
        )  # size_z x (totxN_pool) x latent

        # Transform back
        z = z.view(
            z.shape[0], tot, size_idx, z.shape[2]
        )  # size_z x tot x N_pool x latent
        z = z.clone().detach()

        if self.Infer_model.flag_LV:
            raise RuntimeError("flag_LV must be False")
        else:
            decode_ = self.Infer_model.sample_X(
                z, W_sample
            )  # N_w x Nz x tot x N_pool x obs
        decode_ = decode_.clone().detach()
        # Reduce sample size to save memory
        perm1, perm2 = (
            torch.randperm(decode_.shape[0])[0:10],
            torch.randperm(decode_.shape[1])[0:5],
        )

        decode_ = reduce_size(
            decode_, perm1, perm2
        )  # red_w x red_z x tot x N_pool x obs
        if self.Infer_model.flag_LV:
            raise RuntimeError("flag_LV must be False")
        decode_phi = decode_[:, :, :, :, target_dim]  # red_w x red_z x tot x N_pool

        decode_phi = decode_phi.view(
            -1, decode_phi.shape[2], decode_phi.shape[3]
        )  # (w x z) x tot x N_pool

        comp_mean = torch.unsqueeze(decode_phi, dim=0).repeat(
            decode_phi.shape[0], 1, 1, 1
        )  # (wxz) x (wxz) x tot x N_pool
        ...
        if self.Infer_model.flag_LV:
            ...
            log_likelihood = (
                -0.5 * math.log(2 * np.pi)
                - 0.5 * math.log(self.sigma_out**2)
                - 1.0 / (2 * self.sigma_out**2) * (decode_phi_exp - comp_mean) ** 2
            )  # (wxz) x (wxz) x tot x N_pool
        marginal_log_likelihood = torch.logsumexp(log_likelihood, dim=1) - math.log(
            float(decode_phi_exp.shape[0])
        )  # (w x z) x tot x N_pool
    ...
    def _H_X_Phi_W_id_O(
        ...
        # Reduce sample size
        perm1, perm2 = (
            torch.randperm(decode_.shape[0])[0:20],
            torch.randperm(decode_.shape[1])[0:5],
        )
        ...
        comp_mean = torch.unsqueeze(decode_phi, dim=1).repeat(
            1, decode_phi.shape[1], 1, 1, 1
        )  # W x z x z x tot x N_pool
    ...
    def base_active_learning_decoder(
        self,
        coef=0.999,
        balance_prop=0.25,
        temp_scale=0.25,
        ...
        **kwargs,
        ...
        step = kwargs[
            "step"
        ]  # This is to define how many data points to select for each active learning
        sigma_out = kwargs["sigma_out"]
        W_sample = kwargs["W_sample"]
        sample_z_num = 10
        ...
        if selection_scheme == "overall":
            print(
                "%s selection scheme with Select_Split %s"
                % (selection_scheme, Select_Split)
            )
        else:
            raise NotImplementedError
        ...
        if split > 1 and split < pool_data.shape[0]:
            ...
            for counter_idx in range(split + 1):
                ...
                # Sample X_id
                z, _ = self.model.sample_latent_variable(
                    observed_train_split, observed_train_mask, size=sample_z_num
                )  # size_z x N_pN x latent
                ...
                if strategy == "Opposite_Alpha":
                    if self.Infer_model.flag_LV:
                        ...
                        H_xid_xo = self._H_Xid_Xo(
                            decode, decode.clone().detach(), sigma_out
                        )  # N x obs

                        ############### Sum of I ########################
                        # Compute H[p(x_id|W,x_o)]
                        H_xid_w_xo = self._H_X_id_W_Xo(
                            decode, torch.tensor(decode.data), sigma_out, split=None
                        )  # N_w x N x obs
                    # Compute E[H[p(x_id|W,x_o)]]
                    E_H_xid_w_xo = torch.mean(H_xid_w_xo, dim=0)  # N x obs
                    for row_id in range(observed_train_split.shape[0]):
                        E_H_Phi_id = self._H_X_Phi_O_id(
                            decode, observed_train_split[row_id, :], W_sample, row_id
                        )  # obs
                        E_H_Phi_W_id = self._H_X_Phi_W_id_O(
                            decode, observed_train_split[row_id, :], W_sample, row_id
                        )  # obs
                        if row_id == 0:
                            E_loss_2 = torch.unsqueeze(E_H_Phi_id - E_H_Phi_W_id, dim=0)
                        else:
                            E_loss_2_comp = torch.unsqueeze(
                                E_H_Phi_id - E_H_Phi_W_id, dim=0
                            )
                            E_loss_2 = torch.cat((E_loss_2, E_loss_2_comp), dim=0)

                    BALD_split = (
                        coef * (H_xid_xo - E_H_xid_w_xo) + (1.0 - coef) * E_loss_2
                    )  # N_split x obs
                elif strategy == "MC":
                    if self.Infer_model.flag_LV:
                        raise RuntimeError("flag_LV must be False")
        ...
        if selection_scheme == "overall":
            if not flag_initial:
                ...
                if Select_Split == True:
                    ...
                    BALD[pool_mask_Byte] = 0.0
                    BALD[BALD < 0.0] = 0.0
                    step_new = int((1.0 - balance_prop) * step)

                    # Select from the new user
                    # Apply softmax

                    # Tempering a little
                    BALD_unobserved = assign_zero_row_2D_with_target_reverse(
                        BALD, observed_train
                    )
                    mean_scale = torch.mean(BALD_unobserved[BALD_unobserved > 0.0])
                    temp = (temp_scale + 0.35) * mean_scale
                    BALD_unobserved[BALD_unobserved > 0.0] = torch.clamp(
                        F.softmax(BALD_unobserved[BALD_unobserved > 0.0] / temp),
                        max=1.0,
                        min=1e-10,
                    )

                    BALD_weight_unob = torch.squeeze(BALD_unobserved.view(1, -1))
                    idx_un, flag_full_un, select_num_un = BALD_Select_Explore(
                        BALD_weight_unob, step_new
                    )
                    if not flag_full_un:
                        idx_selected_unobserved = self._transform_idx_pure(
                            idx_un, BALD.shape[1]
                        )
                    ...
                    mean_scale = torch.mean(BALD_observed[BALD_observed > 0.0])
                    temp = temp_scale * mean_scale
                    # Apply softmax
                    BALD_observed[BALD_observed > 0.0] = torch.clamp(
                        F.softmax(BALD_observed[BALD_observed > 0.0] / temp),
                        max=1.0,
                        min=1e-10,
                    )
                    ...
                    idx_ob, flag_full_ob, select_num_ob = BALD_Select_Explore(
                        BALD_weight_ob, step_old
                    )

                    if not flag_full_ob:
                        # Still has remaining data in pool
                        idx_selected_observed = self._transform_idx_pure(
                            idx_ob, BALD.shape[1]
                        )
                    else:
                    ...
                    if not flag_full_un:
                        ...
                        if not flag_full_ob:
                            idx_selected = torch.cat(
                                (idx_selected_unobserved, idx_selected_observed), dim=0
                            )
                        else:
                    ...
                    mean_scale = torch.mean(BALD_[BALD_ > 0.0])
                    temp = (temp_scale + 0.0) * mean_scale
                    BALD_[BALD_ > 0.0] = torch.clamp(
                        F.softmax(BALD_[BALD_ > 0.0] / temp), max=1.0, min=1e-10
                    )
                    BALD_weight_ = torch.squeeze(BALD_.view(1, -1))

                    idx_, flag_full, select_num_ = BALD_Select_Explore(
                        BALD_weight_, step
                    )
                ...
                idx_selected, flag_full = self._transform_idx(
                    idx, BALD.shape[0], BALD.shape[1], num, step=step
                )  # (Nxobs) x 2

            # Apply to observed_train
            if not flag_full:
                num_selected = idx_selected.shape[0]
                observed_train, pool_data = self._apply_selected_idx(
                    observed_train, pool_data, idx_selected
                )
            else:
...
def train_SGHMC(
    ...
    **kwargs,
    ...
    RMSE_mat, MAE_mat, NLL_mat = (
        np.zeros(int(tot_epoch / sample_int)),
        np.zeros(int(tot_epoch / sample_int)),
        np.zeros(int(tot_epoch / sample_int)),
    )
    ...
    for ep in range(tot_epoch):
        ...
        for _, data in enumerate(train_loader):
            ...
            if (ep == 0 or (ep + 1) % hyper_param_update == 0) and flag_update == True:
                ...
                if flag_hybrid:
                    W_dict, r, M, V = Infer_model.sample_pos_W(
                        observed_train,
                        eps=eps,
                        tot_iter=1,
                        thinning=200,
            ...
            if flag_hybrid:
                ELBO, ELBO_mean = Infer_model._compute_target_ELBO(
                    data,
                    mask,
                    mask_target_ELBO,
                    observed_train_size * scale_data,
                    W_dict,
                    size_Z=10,
                    sigma_out=sigma_out,
                    z_sigma_prior=1.0,
                ...
                ELBO, ELBO_mean = Infer_model._compute_ELBO(
                    data,
                    mask,
                    observed_train_size * scale_data,
                    W_dict,
            ...
            W_dict, r, V, M = Infer_model._SGHMC_step(
                W_dict=W_dict,
                G_dict=G_dict,
                eps=eps,
                eps2=eps,
        ...
        if (ep + 1) % thinning == 0 and not flag_optimizer:
            sample_counter += 1
            W_sample = Update_W_sample(
                W_dict, W_sample, sample_counter, maxsize=max_sample_size
            )

        if (ep + 1) % sample_int == 0 and flag_results == True:
            RMSE, MAE, NLL = Test_UCI_batch(
                Infer_model.model,
                test_input_tensor,
                test_target_tensor,
                sigma_out_scale=sigma_out,
                split=10,
                flag_model="PNP_SGHMC",
                size=10,
                Infer_model=Infer_model,
                W_sample=W_sample,
            )
            (
                RMSE_mat[int(ep / sample_int)],
                MAE_mat[int(ep / sample_int)],
                NLL_mat[int(ep / sample_int)],
            ) = RMSE.cpu().data.numpy(), MAE.cpu().data.numpy(), NLL.cpu().data.numpy()

        # Evaluate the validation NLL
        if (ep + 1) % 10 == 0 and valid_data is not None:
            mask_valid_NLL = get_mask(valid_data)
            mean_valid_NLL, _ = Infer_model.test_log_likelihood(
                valid_data,
                valid_data_target,
                W_sample,
                mask_valid_NLL,
            ...
            if counter_valid_break > 15:
                ...
                print(
                    "validation worse, break at ep:%s with test NLL:%s"
                    % (ep + 1, NLL.cpu().data.numpy())
                )
                break
        ...
        if (ep + 1) % 100 == 0:
            ...
            if not flag_imputation:
                mean_train_NLL, _ = Infer_model.test_log_likelihood(
                    observed_train,
                    observed_train.clone().detach(),
                    W_sample,
                    mask_train_NLL,
                    sigma_out=sigma_out,
                    size=10,
            ...
            if valid_data is not None:
                ...
                print(
                    "conditional_coef:%s ep:%s train_NLL:%s NLL:%s RMSE:%s"
                    % (
                        conditional_coef,
                        ep + 1,
                        mean_train_NLL,
