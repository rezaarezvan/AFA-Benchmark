...
class EDDI_Training(nn.Module):
    ...
    def fit(
        ...
        for epoch in trange(epochs, desc="Epochs", unit="epoch"):
            ...
            self.train()
            train_results = {"loss_vae": 0.0, "loss_clf": 0.0, "loss_total": 0.0}
            for features, labels in train_loader:
                ...
                augmented_masked_features, augmented_feature_mask, _ = mask_data(
                    augmented_features, p=p
                )
                encoding, mu, logvar, z, estimated_features = self.partial_vae(
                    augmented_masked_features, augmented_feature_mask
                )
                total_vae_loss, recon_loss, label_loss, kl_loss = self.partial_vae_loss(
                    estimated_features, augmented_features, mu, logvar, epoch
                )
                logits = self.classifier(z)
                classifier_loss = F.cross_entropy(logits, labels)
                total = (
                    total_vae_loss + classifier_loss_scaling_factor * classifier_loss
                )
                total.backward()
                optimizer.step()
            ...
            with torch.no_grad():
                for features, labels in val_loader:
                    ...
                    for suffix, p in [("many", min_mask), ("few", max_mask)]:
                        ...
                        _, mu, logvar, z, estimated_features = self.partial_vae(
                            augmented_masked_features, augmented_feature_mask
                        )
                        total_vae_loss, _, _, _ = self.partial_vae_loss(
                            estimated_features, augmented_features, mu, logvar, epoch
                        )
                        logits = self.classifier(z)
                        classifier_loss = F.cross_entropy(logits, labels)
                        ...
                        eval_results[f"loss_total_{suffix}"] += (
                            total_vae_loss + classifier_loss
                        ).item()
            ...
            wandb.log(
                {
                    ...
                    "joint_training/val_loss_many": eval_results["loss_total_many"],
                    "joint_training/val_loss_few": eval_results["loss_total_few"],
                    "joint_training/val_acc_many": eval_results["acc_many"],
                    "joint_training/val_acc_few": eval_results["acc_few"],
                }
            )

        wandb.unwatch([self.classifier, self.partial_vae])

    def partial_vae_loss(
        self,
        ...
        label_recon_loss = F.cross_entropy(
            estimated_label_logits, labels, reduction="mean"
        )
        feature_recon_loss = (
