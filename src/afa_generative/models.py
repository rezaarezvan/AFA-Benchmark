...
class PartialVAE(nn.Module):
    ...
    def fit(
        ...
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(
            opt, factor=factor, patience=patience, min_lr=min_lr
        )
...
class fc_Net(nn.Module):
    ...
    def _get_W_dict(self):
        ...
        if self.flag_only_output_layer == False:
            for layer_ind in range(self.hidden_layer_num):
                W_dict["weight_layer_%s" % (layer_ind)] = (
                    self.hidden[layer_ind].weight.clone().detach()
                )
                W_dict["bias_layer_%s" % (layer_ind)] = (
                    self.hidden[layer_ind].bias.clone().detach()
                )
        ...
        if self.flag_only_output_layer == False:
            for layer_ind in range(self.hidden_layer_num):
                if self.hidden[layer_ind].weight.grad is not None:
                    G_dict["weight_layer_%s" % (layer_ind)] = (
                        -self.hidden[layer_ind].weight.grad.clone().detach()
                    )
                    G_dict["bias_layer_%s" % (layer_ind)] = (
                        -self.hidden[layer_ind].bias.grad.clone().detach()
                    )
    ...
    def _flatten_stat(self):
        if self.flag_only_output_layer == False:
            ...
            Out_weight, Out_b_weight = (
                self.out.weight,
                self.out.bias,
            )  # N_in x N_out or N_out

            if self.flag_LV == True:
                W_weight_LV, b_weight_LV = self.out_LV.weight, self.out_LV.bias
                Out_weight, Out_b_weight = (
                    torch.cat((Out_weight, W_weight_LV), dim=1),
                    torch.cat((Out_b_weight, b_weight_LV), dim=0),
                )  #' This should be N_in x (2 x N_out) or (2 X N_out)'
...
class Point_Net_Plus_BNN_SGHMC(object):
    def __init__(
        ...
        encoder_hidden_before_agg=None,
        encoder_layer_num_after_agg=1,
        encoder_hidden_after_agg=[100],
        embedding_dim=10,
        decoder_layer_num=2,
        decoder_hidden=[50, 100],
        pooling="Sum",
        output_const=1.0,
        add_const=0.0,
        sample_z=1,
        ...
        if self.pooling_act == "ReLU":
            ...
            raise NotImplementedError
        self.encode_embedding, self.encode_bias = (
            self._generate_default_embedding()
        )  # 1 x obs_dim x embed_dim and 1 x obs_dim x 1

        # BNN Decoder network
        self.decoder = fc_Net(
            self.latent_dim,
            self.obs_dim,
            hidden_layer_num=decoder_layer_num,
            hidden_unit=decoder_hidden,
            activations="ReLU",
            activations_output=None,
            flag_only_output_layer=False,
            drop_out_rate=0.0,
        ...
        if type(self.encoder_layer_num_before_agg) == type(None):
            ...
            self.encoder_before_agg = fc_Net(
                self.embedding_dim + 2,
                output_dim=self.dim_before_agg,
    ...
    def _extract_state_dict(self):
        ...
        embedding_state, embedding_bias = (
            self.encode_embedding.clone().detach(),
            self.encode_bias.clone().detach(),
        )
        return (
            decoder_state,
            encoder_before_state,
            encoder_after_state,
            embedding_state,
            embedding_bias,
        )
    ...
    def _encoding(self, X, mask):
        ...
        if self.flag_encoder_before_agg == True:
            X_aug = torch.cat(
                (X_expand, X_embedding, X_bias), dim=len(X_expand.shape) - 1
            )  # N x obs_dim x (embed+2)
            output_before_agg = 1 * self.encoder_before_agg.forward(
                X_aug
            )  # N x obs x dim_before_agg
        else:
        ...
        if self.pooling != "Max":
            agg_mask_output = self.pool(
                mask_output_before_agg, dim=len(mask_output_before_agg.shape) - 2
            )
            if self.act_pool:
                agg_mask_output = self.act_pool(agg_mask_output)  # N x dim_before_agg
        elif self.pooling == "Max":
            agg_mask_output = torch.max(
                mask_output_before_agg, dim=len(mask_output_before_agg.shape) - 2
            )[0]
            if self.act_pool:
        ...
        # Encoding -3
        encoding[:, self.latent_dim :] = encoding[:, self.latent_dim :] - 0.0
        return encoding
    ...
    def sample_latent_variable(self, X, mask, size=10):
        ...
        if self.flag_log_q == True:
            sigma = torch.clamp(
                torch.sqrt(torch.exp(encoding[:, self.latent_dim :])),
                min=1e-5,
                max=300.0,
            )
        else:
            sigma = torch.clamp(
                torch.sqrt(
                    torch.clamp((encoding[:, self.latent_dim :]) ** 2, min=1e-8)
                ),
                min=1e-5,
                max=300.0,
            )
        ...
        if size == 1:
            ...
            eps = torch.randn(
                size, batch_size, self.latent_dim, requires_grad=True
            )  # size x N  x latent_dim
            z = (
                torch.unsqueeze(mean, dim=0) + torch.unsqueeze(sigma, dim=0) * eps
            )  # size x N x latent_dim
        else:
            raise NotImplementedError
        return z, encoding

    def _KL_encoder(self, mean, sigma, z_sigma_prior=1.0):
        KL_z = 0.5 * (
            math.log(z_sigma_prior**2)
            - torch.sum(torch.log(sigma**2), dim=1)
            - sigma.shape[1]
            + torch.sum(sigma**2 / (z_sigma_prior**2), dim=1)
            + torch.sum(mean**2 / (z_sigma_prior**2), dim=1)
        )  # N

        return KL_z  # N

    def test_log_likelihood(
        self, Infer_model, X_in, X_test, W_sample, mask, sigma_out, size=10
    ):
        mean_pred_log_likelihood, tot_pred_ll = Infer_model.test_log_likelihood(
            X_in, X_test, W_sample, mask, sigma_out, size=size
        )
        return mean_pred_log_likelihood, tot_pred_ll

    def completion(
        self, Infer_model, X, mask, W_sample, size_Z=10, record_memory=False
    ):
        complete = Infer_model.completion(
            X, mask, W_sample, size_Z=size_Z, record_memory=False
        )
        return complete

    def _KL_encoder_target_ELBO(self, q_mean_XUy, q_sigma_XUy, q_mean_Xdy, q_sigma_Xdy):
        # clamp to avoid zero
        sigma = torch.clamp(torch.abs(q_sigma_XUy), min=0.001, max=10.0)
        sigma_Xdy = torch.clamp(torch.abs(q_sigma_Xdy), min=0.001, max=10.0)
        KL_z = (
            torch.log(torch.abs(sigma_Xdy) / torch.abs(sigma))
            + 0.5 * (sigma**2) / (sigma_Xdy**2)
            + 0.5 * ((q_mean_XUy - q_mean_Xdy) ** 2) / (sigma_Xdy**2)
            - 0.5
        )  # N x latent_dim
