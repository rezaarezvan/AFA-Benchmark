...
import yaml
import time
from afa_discriminative.scripts.pretrain_gadgil2023 import main as pretrain_main
from afa_discriminative.scripts.train_gadgil2023 import main as train_main
        ...
        for dataset_type in dataset_types:
            for split in splits:
                for seed in seeds:
                    ...
                    rprint(
                        f"[bold blue]Memory used before script: {torch.cuda.memory_allocated() / 1024**2:.2f} MB[/bold blue]"
                    )
                    rprint(
                        f"[bold green]Pretraining with dataset_type={dataset_type}, split={split}, seed={seed}[/bold green]"
                    )
                    pretrain_main(
                        pretrain_config_path=pretrain_config_path,
                        dataset_type=dataset_type,
                        train_dataset_path=Path(
                            f"data/{dataset_type}/train_split_{split}.pt"
                        ),
                        val_dataset_path=Path(
                            f"data/{dataset_type}/val_split_{split}.pt"
                        ),
                    ...
                    rprint(
                        f"[bold blue]Memory used after script: {torch.cuda.memory_allocated() / 1024**2:.2f} MB[/bold blue]"
                    )

        rprint(
            "[bold magenta]Pretraining finished, now starting training.[/bold magenta]"
        )
            ...
            for split in splits:
                for seed in seeds:
                    ...
                    pretrained_model_folders = get_folders_with_matching_params(
                        folder=pretrained_model_folder,
                        mapping={
                            "dataset_type": dataset_type,
                            "train_dataset_path": str(
                                Path(f"data/{dataset_type}/train_split_{split}.pt")
                            ),
                            "val_dataset_path": str(
                                Path(f"data/{dataset_type}/val_split_{split}.pt")
                            ),
                            "seed": seed,
                        },
                    )
                    assert len(pretrained_model_folders) == 1, (
                        f"Found {len(pretrained_model_folders)} pretrained model folders matching the parameters, expected 1."
                    )
                    ...
                    for hard_budget in hard_budgets[dataset_type]:
                        ...
                        rprint(
                            f"[bold green]Training with dataset_type={dataset_type}, split={split}, seed={seed}, hard_budget={hard_budget}[/bold green]"
                        )
                        train_main(
                            pretrain_config_path=pretrain_config_path,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(f"models/pretrained/gadgil2023/{timestr}"),
        help="Path to the folder containing pretrained models",
    )
    parser.add_argument(
        "--method_folder",
        type=Path,
        default=Path(f"models/methods/gadgil2023/{timestr}"),
        help="Path to the folder containing method models",
    )
    parser.add_argument(
        "--pretrain_config_path",
        type=Path,
        default=Path("configs/gadgil2023/pretrain_gadgil2023.yml"),
        help="Path to the pretrain configuration file",
    )
    parser.add_argument(
        "--train_config_path",
        type=Path,
        default=Path("configs/gadgil2023/train_gadgil2023.yml"),
        help="Path to the train configuration file",
    )
    parser.add_argument(
        "--pipeline_config_path",
        type=Path,
        default=Path("configs/gadgil2023/pipeline.yml"),
