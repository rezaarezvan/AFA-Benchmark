    ...
    if method == "cae":
        ...
        diff.fit(
            ...
            val_loader=DataLoader(
                TensorDataset(val_ds.features, val_ds.labels),
                batch_size=1024
            ),
            lr=1e-3, nepochs=250, loss_fn=torch.nn.CrossEntropyLoss(),
        ...
        if len(unique) != budget:
            ...
            selected = np.sort(np.concatenate([unique, remaining_features[:num_extras]]))
        ...
        base.fit(
            ...
            lr=1e-3, nepochs=250, loss_fn=torch.nn.CrossEntropyLoss(), verbose=False
        )
        # compute importances
        auroc = lambda p,y: AUROC(task="multiclass", num_classes=y.max().item()+1)(p.softmax(1), y)
        permutation_importance = np.zeros(d_in)
        for f in range(d_in):
            X_val = val_ds.features.clone()
            X_val[:,f] = train_ds.features[torch.randint(len(train_ds), (len(X_val),)), f]
            with torch.no_grad():
...
def main(args):
    ...
    for method in STATIC_METHOD_REGISTRY:
        ...
        train_ds: AFADataset = AFA_DATASET_REGISTRY[args.dataset_type].load(paths["train"])
        ...
        if s<1 or e<s:
            raise ValueError(f"Invalid budget range: start_feature_num={s}, end_feature_num={e}")
        e = min(e, d_in)
        budgets = list(range(s, e+1))

        selected_history = []
        for b in tqdm(budgets, desc=f"{method}-{args.dataset_type}-split{args.split}"):
            sel = compute_selected_for_budget(method, d_in, d_out, train_ds, val_ds, b, device)
            selected_history.append(sel)

        # save
        out = args.results_folder / f"{method}_{args.dataset_type}_split{args.split}_s{s}_e{e}"
...
if __name__=="__main__":
    ...
    parser.add_argument("--dataset_type", required=True, choices=AFA_DATASET_REGISTRY.keys())
    parser.add_argument("--start_feature_num", type=int, default=1)
    parser.add_argument("--end_feature_num", type=int, required=True)
    parser.add_argument("--results_folder", type=Path, default=Path("models/static/selected"))
