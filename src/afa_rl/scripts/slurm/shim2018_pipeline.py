...
def wait_for_jobs(job_ids: list[int]) -> None:
    ...
    with tqdm(total=len(job_ids), desc="Jobs progress") as pbar:
        ...
        while len(completed_jobs) < len(job_ids):
            ...
            pbar.set_description(f"Jobs progress (Elapsed: {hours:02}:{minutes:02}:{seconds:02})")
        ...
        for dataset_type in dataset_types:
            for split in splits:
                for seed in seeds:
                    ...
                    assert result.returncode == 0, f"Error submitting job: {result.stderr}"
                    output = result.stdout.strip()
                    job_id = output.split()[-1]  # Extract the job ID from the output
                    print(f"Submitted pretraining job with ID {job_id} for dataset_type={dataset_type}, split={split}, seed={seed}")
                ...
                for seed in seeds:
                    ...
                    pretrained_model_folders = get_folders_with_matching_params(
                        pretrained_model_folder,
                        {
                            ...
                            "train_dataset_path": "data/{dataset_type}/train_split_{split}.pt",
                            "val_dataset_path": "data/{dataset_type}/val_split_{split}.pt",
                            "seed": seed
                        }
                    )
                    # There should be exactly one pretrained model
                    assert len(pretrained_model_folders) == 1, f"Found {len(pretrained_model_folders)} pretrained models for dataset_type={dataset_type}, split={split}, seed={seed}. Expected 1."
                    ...
                    for hard_budget in hard_budgets[dataset_type]:
                        ...
                        result = subprocess.run(
                            [
                                "sbatch",
                                f"--export={env_vars}",
                                "src/afa_rl/scripts/slurm/train_shim2018_job.sh",
                            ],
                            stdout=subprocess.PIPE,
                            stderr=subprocess.PIPE,
                            text=True
                        ...
                        print(f"Submitted training job with ID {job_id} for dataset_type={dataset_type}, split={split}, seed={seed}, hard_budget={hard_budget}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--pretrained_model_folder",
        type=Path,
        default=Path(f"models/pretrained/shim2018/{time.strftime('%Y-%m-%d')}"),
        help="Path to the folder containing pretrained models",
    )
    parser.add_argument(
        "--method_folder",
        type=Path,
        default=Path("models/methods/shim2018"),
        help="Path to the folder containing method models",
    )
    parser.add_argument(
        "--pretrain_config_path",
        type=Path,
        default=Path(f"models/methods/shim2018/{time.strftime('%Y-%m-%d')}"),
