...
def get_eval_metrics(
    ...
    for td in eval_tds:
        ...
        td_end = td[-1:]
        logits = masked_classifier(td_end["next","masked_features"], td_end["next", "feature_mask"])
...
def main(
    ...
    pretrained_model_optim = optim.Adam(
        masked_classifier.parameters(), lr=train_config.pretrained_model.lr
    )
    ...
    check_masked_classifier_performance(
        masked_classifier=masked_classifier,
        dataset=val_dataset,
    ...
    try:
        for batch_idx, td in tqdm(
            ...
            if batch_idx >= train_config.activate_joint_training_after_n_batches:
                ...
                logits = masked_classifier(td["masked_features"], td["feature_mask"])
                class_loss = F.cross_entropy(
                    logits, td["label"], weight=class_weights
                )
            ...
            if batch_idx != 0 and batch_idx % train_config.eval_every_n_batches == 0:
                ...
                metrics_eval = get_eval_metrics(td_evals, masked_classifier)
                benchmark_metrics_eval = get_eval_metrics(
                    benchmark_td_evals, masked_classifier
                )
                run.log(
                    {
                        **{
                            f"eval/{k}": v
                            for k, v in (metrics_eval | agent.get_eval_info() | {"classifier_norm": module_norm(masked_classifier)}).items()
                        },
                        **{
